...
The numerical issue is resolved: all18 predeclared cases (six representations × workers0/2/4) passed the original deterministic parity, resume, state and gradient checks. Timings use the ordinary production backend. All models, features, splits and numerical tolerances remain unchanged. Full production capture and scientific fits have **not** been submitted. Work is waiting for the user's explicit scope/budget choice, not a new authentication or numerical failure.
...
Next action: wait for the explicit scope/budget answer, preserve the selected matrix in a new immutable protocol/workflow, then launch only that reviewed production chain. Do not activate the old14-fit dispatcher or silently release remaining candidates. Checkpoints, metadata and terminal receipts must survive laptop closure; no numerical work moves to the MacBook.
