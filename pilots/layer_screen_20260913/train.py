...
def _fit(cache, run_dir, arm, seed, device):
    ...
    try:
        if start_epoch < config["training"]["minimum_epochs"] or stale < config["training"]["patience"]:
            for epoch in range(start_epoch, config["training"]["epochs"]):
                ...
                if epoch + 1 >= config["training"]["minimum_epochs"] and stale >= config["training"]["patience"]:
                    break
                if _STOP_REQUESTED:
...
def main():
    ...
    if args.device != "cuda":
        parser.error("This screen requires a Slurm CUDA allocation")
