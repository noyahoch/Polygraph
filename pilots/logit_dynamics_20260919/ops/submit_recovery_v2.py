...
def main():
    ...
    if sha(__file__) != config["operator_submit_sha256"]:
        raise RuntimeError("Recovery submit helper differs from reviewed hash")
    gate = root / "evaluation_gate.json"
    if sha(gate) != config["operational_recovery"]["evaluation_gate_sha256"] or json.loads(gate.read_text()).get("complete") is not True:
        raise RuntimeError("The completed all-three-fit evaluation gate changed")
