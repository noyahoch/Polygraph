...
Polygraph has the highest mean AUROC at all five severities; the graph expert also exceeds the output MLP at each (Table~\ref{tab:results_severity}).
...
Table~\ref{tab:ablations} reports the single-seed TransformerConv validation study. Class direction improves base-node performance; magnitude alone adds little. With hidden nodes fixed, full evidence-flow edges improve over raw attention.
...
These exploratory studies reuse 800 previously examined, non-error-balanced development photographs, with nine grouped views: clean and Gaussian noise, motion blur, fog and JPEG at severities 3/5. We report mean AUROC over seeds 7/17/27.

\paragraph{Models.}
$G/S$ average four preserved graph/set detectors at attention layers 3/6/9/12, trained for 20 epochs on 1,600 photographs with 400 for selection. Both receive H12-based 784-dimensional node values and 12 attention-edge values; sets discard incidence. The fixed ViT-B LogitDynamics adaptation ($D$)~\citep{Beigelman2026} uses 12 auxiliary CLS class heads and a linear error readout. It allocates 1,200/800/400 photographs to head fitting/readout fitting/checkpoint selection, with 16/100 training epochs and validation-AP readout selection. Budgets and supervision differ from $G/S$; these are not comparisons with original conditional Polygraph.

\paragraph{Complementarity.}
Standardized L2 logistic combiners are fitted on a fixed 400-photo subset and evaluated on the other 400, unlike the original mixture gate. $G/S$ are mean probabilities; $D$ is its preserved error logit. $D'$ pairs another LD seed cyclically; pairs overlap. Table~\ref{tab:followup-fusion} shows $D$ outperforming $G/S$ individually, yet $DG-D=+0.007203$ (adjusted interval $[0.003415,0.011330]$). The graph--set fusion difference is $+0.000043$ ($[-0.001311,0.001508]$): no demonstrated graph-specific advantage, not equivalence. The two-seed LD control is not compute-matched.
...
\begin{table}[!htbp]
  ...
  \begin{tabularx}{\columnwidth}{@{}XrXr@{}}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Mean AUROC on 400 fusion-assessment photographs, distinct from the original benchmark and 800-photo readout evaluation.}
  \label{tab:followup-fusion}
\end{table}

\paragraph{LD readout ablation.}
On all 800 evaluation photographs, six original-classifier features yield 0.8636; adding the final-layer auxiliary block ($B$) yields 0.8948; adding intermediate numeric blocks ($C$), 0.8990. With heads fixed and the same readout recipe, $C-B=+0.004135$ ($[0.000428,0.007911]$). The preserved full $D$, including class-identity dynamics, achieves 0.8981. The six-feature baseline is not the full-logit MLP; these increments do not causally explain the LD--GNN gap.

\paragraph{Uncertainty.}
We use 2,000 paired photo-group bootstrap draws shared across methods/seeds. Fusion resamples fitting and assessment photographs, refitting combiners; the ablation resamples evaluation only. Bonferroni adjustment across $DG-D$, $DG-DS$ and $C-B$ gives individual $98\frac{1}{3}\%$ percentile intervals. These condition on base detectors or fitted readouts, respectively; they do not correct development-data exposure.
